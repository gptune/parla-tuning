...
def main():
    ...
    tuning_metadata = {
        "tuning_problem_name": "GPTUNE-SEARCH-failure_handling_"+str(failure_handling)+"-n_rows_"+str(n_rows)+"-n_cols_"+str(n_cols)+"-mattype_"+str(mattype)+"-npilot_"+str(npilot)+"-batch_num_"+str(batch_num),
        "historydb_path": "gptune.db",
    ...
    if npilot == 0:
        ...
        with open("lhsmdu.sample/LHSMDU-SAMPLE-n_rows_"+str(n_rows)+"-n_cols_"+str(n_cols)+"-n_samples_"+str(npilot)+"-batch_num_"+str(batch_num)+".json") as f_in:
            ...
            for sample in samples:
                P_ = [sample["rls_method"], sample["sketch_operator"], sample["sampling_factor"], sample["vec_nnz"], sample["safety_exponent"]]
                P.append(P_)
