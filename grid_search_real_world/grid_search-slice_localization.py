...
def objective(params):
    ...
    grid_search_logfile = "grid_search.db/GRID-SEARCH-SLICE_LOCALIZATION-n_rows_"+str(n_rows)+"-n_cols_"+str(n_cols)+"-tolerance_"+str(tolerance)+".json"
    json_data_arr = []
...
def run_grid_search():
    ...
    A = np.genfromtxt("../input/slice_localization/slice_localization-data-nrows_"+str(n_rows)+"-ncols_"+str(n_cols)+".csv", delimiter=',', skip_header=1, dtype=np.float64)
    A = np.delete(A, 0, 1) # the first row of the synthetic input data is meta information, so we remove that here.

    b = np.genfromtxt("../input/slice_localization/slice_localization-result-nrows_"+str(n_rows)+"-ncols_"+str(n_cols)+".csv", delimiter=',', skip_header=1, dtype=np.float64)
