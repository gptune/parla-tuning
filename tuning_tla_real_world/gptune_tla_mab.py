...
def objectives(point):
    ...
    print ("objective function reference_normalized_residual_error_to_Axstar: ", reference_normalized_residual_error_to_Axstar)
            ...
            def LoadCategory():
                ...
                with open("lhsmdu.db/LHSMDU-SEARCH-failure_handling_"+str(source_failure_handling)+"-n_rows_"+str(source_n_rows)+"-n_cols_"+str(source_n_cols)+"-dataset_"+str(source_dataset)+"-batch_num_1.json") as f_in:
                    ...
                    for i in range(len(function_evaluations)):
                        ...
                        wall_clock_time = np.average(func_eval["additional_output"]["parla_times"])
                        normalized_residual_error_to_Axstar = np.average(func_eval["additional_output"]["normalized_residual_errors_to_Axstar"])
                        if func_eval["tuning_parameter"]["rls_method"] == "QR-LSQR" and func_eval["tuning_parameter"]["sketch_operator"] == "sjlt":
                            category = 0
                        elif func_eval["tuning_parameter"]["rls_method"] == "SVD-LSQR" and func_eval["tuning_parameter"]["sketch_operator"] == "sjlt":
                            category = 1
                        elif func_eval["tuning_parameter"]["rls_method"] == "SVD-PGD" and func_eval["tuning_parameter"]["sketch_operator"] == "sjlt":
                            category = 2
                        elif func_eval["tuning_parameter"]["rls_method"] == "QR-LSQR" and func_eval["tuning_parameter"]["sketch_operator"] == "less_uniform":
                            category = 3
                        elif func_eval["tuning_parameter"]["rls_method"] == "SVD-LSQR" and func_eval["tuning_parameter"]["sketch_operator"] == "less_uniform":
                            category = 4
                        elif func_eval["tuning_parameter"]["rls_method"] == "SVD-PGD" and func_eval["tuning_parameter"]["sketch_operator"] == "less_uniform":
                            category = 5
...
def RunTLA_TargetGreedy(nrun, trials):
    ...
    with open("gptune_tla.db/GPTUNE-TLA_TargetGreedyTrials"+str(trials)+"-target-failure_handling_"+str(failure_handling)+"-"+str(n_rows)+"-"+str(n_cols)+"-dataset_"+str(dataset)+"-source-failure_handling_"+str(source_failure_handling)+"-"+str(source_n_rows)+"-"+str(source_n_cols)+"-source_dataset_"+str(source_dataset)+"-batch_num_"+str(batch_num)+".json", "r") as f_in:
        ...
        for i in range(len(function_evaluations)):
            ...
            wall_clock_time = np.average(func_eval["additional_output"]["parla_times"])
            normalized_residual_error_to_Axstar = np.average(func_eval["additional_output"]["normalized_residual_errors_to_Axstar"])
            if func_eval["constants"]["rls_method"] == "QR-LSQR" and func_eval["constants"]["sketch_operator"] == "sjlt":
                category = 0
            elif func_eval["constants"]["rls_method"] == "SVD-LSQR" and func_eval["constants"]["sketch_operator"] == "sjlt":
                category = 1
            elif func_eval["constants"]["rls_method"] == "SVD-PGD" and func_eval["constants"]["sketch_operator"] == "sjlt":
                category = 2
            elif func_eval["constants"]["rls_method"] == "QR-LSQR" and func_eval["constants"]["sketch_operator"] == "less_uniform":
                category = 3
            elif func_eval["constants"]["rls_method"] == "SVD-LSQR" and func_eval["constants"]["sketch_operator"] == "less_uniform":
                category = 4
            elif func_eval["constants"]["rls_method"] == "SVD-PGD" and func_eval["constants"]["sketch_operator"] == "less_uniform":
                category = 5
            evaluations_per_category[category].append(func_eval["evaluation_result"]["obj"])
    average_evaluations_per_category = [np.mean(evaluations_per_category[i]) for i in range(len(evaluations_per_category))]
    print ("average_evaluations_per_category: ", average_evaluations_per_category)
